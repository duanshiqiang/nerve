...
class Nerve::Reporter
  class Zookeepervisenze < Base
    ...
    def initialize(service)
      ...
      @data = service['zk_data'] || parse_data(get_service_data(service))
      if @data.is_a?(Hash)
        @data = @data.to_json
      end
      @zk_path = service['zk_path']
      @zk_key = service['zk_key'] || "#{service['host']}:#{service['port']}"
      @full_key = nil
    end
    ...
    def zk_create
      ...
      @full_key = @zk.create("#{@zk_path}/#{@zk_key}", :data => @data, :mode => :ephemeral)
      log.info "nerve: added zk node at #{@full_key}" if @full_key
    end
